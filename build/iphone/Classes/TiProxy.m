...
//Rare exceptions to indicate a bug in the _cityworks code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_cityworks = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_cityworks]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.cityworks.module"
